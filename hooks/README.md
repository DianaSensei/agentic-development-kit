...
| `check-conventions.sh` | `PostToolUse` on edits | `warn` | Runs the project's own checks - its linters, formatters and rule scripts, listed as `checks` in **its** `.claude/quality-check.config.json` (never the bundled config: running commands is the project's decision) - on the file just written, and hands any failure straight back to Claude while the change is in view. `warn` passes it as context; `block` makes Claude address it before going on. A checker this machine lacks (exit 127) is skipped, not failed. Only `scope: file` checks run here; the workflows run `scope: project` ones with the tests. Format and detection: `project-setup`'s `references/conventions.md`. |
| `quality-gate.sh` | `Stop` | `warn` | Flags - or, set to `block`, refuses to end - a turn that leaves uncommitted **code** changes no review has vouched for, or that were never tested: it wants `quality_gate.review_skill` read and a test run (a Bash command matching `quality_gate.test_command`) after the last code edit. "The tests pass" is the claim an agent most often makes without running them; this checks that they ran, and leaves reporting the result - failures included - to Claude. `require_test_run: false` for a project with no tests. |
...
- `block` - the turn is held and the reason goes to Claude, which names the skill to read, the diff to
  read it against, and that reading it is what releases the gate. This is the mode in which
...
- `checks` - the project's convention checks, `{name, run, match, scope, timeout}`; read only from the
  project's own config. See `check-conventions.sh` above.
...
- `bulk_read_gate.line_threshold` - lines above which a whole-file `Read` is redirected to
  `bulk-reader`. `.exclude` - EREs (repo-relative path) for files that must always be read whole
  regardless of length, e.g. a lockfile you're diffing rather than summarizing. `SKILL.md` ships in
  that list on purpose: `skill_gate` is satisfied only by a genuine `Read` of the file (see
  `skill_ref_pattern`), which a `bulk-reader` summary is not, so with both gates on `block` an
  over-threshold skill would be unreadable and its files unedittable at the same time.
