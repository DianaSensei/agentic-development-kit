...
| [`hooks/`](./hooks/README.md) | Gates for the rules a model cannot self-police: the owning `SKILL.md` was read before an edit, the project's own linters and rule scripts pass on every file Claude writes, `code-review-skill` ran before "done", and a whole-file `Read` past a line threshold routes to the cheap `bulk-reader` agent instead of the expensive model's context. Warn by default, blocking per gate. |
