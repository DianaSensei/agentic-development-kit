...
await client.callTool({
  ...
  arguments: {
    issue_key: "PROJ-123",
    time_spent: "1h 30m",              // Jira duration format: Xw Xd Xh Xm
    started: "2024-01-15T09:00:00.000+0000",  // ISO 8601, when the work began
    comment: "Implemented parameterized query fix; added regression test for the reported input."
  }
});

// Get existing worklogs (e.g. before adding, to avoid duplicate entries for the same session)
const worklogs = await client.callTool({
...
```typescript
const sessionStart = new Date(); // capture when work on the issue actually begins

// ... implementation work happens here ...

async function logSessionWork(
  client: MCPClient,
  issueKey: string,
  sessionStart: Date,
  comment: string
...
function formatJiraDuration(ms: number): string {
