...
**Capabilities:**
- Jira Cloud, Confluence Cloud, Jira Service Management, Bitbucket, Compass
- OAuth 2.1 authentication flow (interactive browser sign-in on first connect, then cached)
- Read/write operations for issues and pages
- JQL and CQL query support

### Open-Source Alternatives

**mcp-atlassian (sooperset)** - Most feature-rich community option, Python-only (no npm package):
```bash
...
**Official Remote Server (via `mcp-remote` proxy):**
```json
{
  "mcpServers": {
    "atlassian": {
...
```bash
# Test the mcp-remote proxy to the official server (opens OAuth browser flow on first run)
