...
Comprehensive guide for managing data across distributed services. For choosing a database
*technology* (not the distributed-data patterns below), see `references/database-selection.md` and,
for depth, `database-skill`.
...
See `references/resilience-patterns.md` for when to choose choreography vs. orchestration as the
saga style. This section covers the state-management mechanics common to both.
