...
metadata:
  ...
  related-skills: architecture-designer, messaging-skill, java-spring-skill, tauri-react-skill, code-review-skill
---
...
1. **Discover** - Read the existing contract (OpenAPI/`.graphql`/`.proto`/AsyncAPI if present) and the naming/versioning/auth conventions already in use. Determine the kind of communication being designed: synchronous (REST/GraphQL/RPC) or asynchronous via a broker (`messaging-skill`). If a distributed-systems-level decision is still open (which services should even talk to each other, sync vs. async as an architecture choice), that's `architecture-designer`'s call to make first - this skill takes it from there to the concrete contract.
