...
Sửa đúng root cause đã được duyệt ở Bước 3 (không sửa theo triệu chứng bề mặt). Trước khi viết bất kỳ dòng code nào thuộc phạm vi 1 skill kỹ thuật đã ánh xạ ở Bước 0 — BẮT BUỘC gọi
`Read` để đọc TOÀN VĂN `SKILL.md` tương ứng NGAY LÚC ĐÓ, không dựa vào tên/mô tả đã cache để suy luận nội dung. Việc cache ở Bước 0 chỉ để biết skill nào tồn tại, không thay thế
việc đọc thật khi áp dụng.

Nếu 1 bug/featue liên quan tới NHIỀU skill (VD: vừa Java/Spring vừa Database), đọc TOÀN BỘ các
