...
| `independent-review/claims-not-in-diff` | a review that believes the change's description: it claims banker's rounding (the diff truncates), added tests (none) and no behavior change for `apply_tax` (the diff changes it). The *Claims* section must mark each not met. Measured with a no-plugin baseline: 1.00 with the kit, 0.57 without - a bare model often catches the claims too, but never in the approver's format and never grading them |
| `conventions/fixes-what-the-check-reports` | a direct edit leaving code that breaks a convention only a project checker knows (logging through `app.log`, not `logging` or `print`) - the `check-conventions` hook must report it and Claude must fix it. Measured with a no-plugin baseline: 1.00 with the kit (the hook fired in every run), 0.33 without |
